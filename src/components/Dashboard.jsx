import "../Css/App.css";

export const Dashboard = () => {
  return (
    <div className="dash">
      <div className="greeting">
        <p>Hello</p>
        <p>!!</p>
      </div>
      <p>
        Welcome to the nc-news webapp, thank you for visiting. Here is a breif
        overview of the site map, and a run down on the basics.
      </p>
      <br />

      <br />
      <p>Where will each of the icons will take you?</p>
      <div className="dash-list">
        <ul>
          <li>
            <img src="data:image/png;base64," />
            <p>Article Editor - For creating and posting new articles.</p>
          </li>
          <li>
            <img src="data:image/png;base64," />
            <p>Main View - Where you can find, filter and view articles.</p>
          </li>
          <li>
            <img src="data:image/png;base64," />
            <p>User Center - under construction!</p>
          </li>
        </ul>
      </div>
      <p>
        Through out the site you will see a few diffrent symbols, I wanted to
        add some visual cues to the site so here is a little info on them.
      </p>

      <div className="symbol-grid">
        <div className="grid-item">
          <p id="grid-symbol">++</p>
          <p id="symbol-info">Votes for an article</p>
        </div>
        <div className="grid-item">
          <p id="grid-symbol">≡</p>
          <p id="symbol-info">Total comments on an article</p>
        </div>

        <div className="grid-item">
          <p id="grid-symbol">¤</p>
          <p id="symbol-info">
            A unique interactivity score for an article or topic
          </p>
        </div>

        <div className="grid-item">
          <p id="grid-symbol">¶</p>
          <p id="symbol-info">Total articles / new article</p>
        </div>
      </div>

      <p>Thank You for visting!</p>
      <br />
      <p> To view the repositories on github they are linekd below.</p>
      <div className="links">
        <a href="https://github.com/jjcrl/fe-nc-news">Front End Repo</a>
        <a href="https://github.com/jjcrl/be-nc-news">Back End Repo </a>
      </div>
    </div>
  );
};
